...
func (app *Config) twilioSendOTP(phoneNumber string) (string, error) {
	...
	resp, err := client.VerifyV2.CreateVerification(envAccountServiceID(), params)
	if err != nil {
		return "", err
	}
...
func (app *Config) twilioVerifyOTP(phoneNumber string, code string) error {
	...
	resp, err := client.VerifyV2.CreateVerificationCheck(envAccountServiceID(), params)
	if err != nil {
		return err
	}
